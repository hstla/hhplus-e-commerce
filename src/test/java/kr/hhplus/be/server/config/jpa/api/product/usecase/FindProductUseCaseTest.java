...
//
// 	@Mock
// 	private UserPointLockManager userService;
//
// 	@InjectMocks
...
// 			willDoNothing().given(userService).deductPointWithLock(userId, totalPrice);
...
// 			then(userService).should(times(1)).deductPointWithLock(userId, totalPrice);
...
//
// 			then(orderService).should(times(1)).payComplete(orderId);
// 			then(userService).should(never()).deductPointWithLock(anyLong(), anyLong());
...
//
// 			given(orderService.payComplete(orderId)).willReturn(orderInfo);
// 			willThrow(new RuntimeException("Point payment failed")).given(userService).deductPointWithLock(userId, totalPrice);
//
// 			// when & then
// 			assertThatThrownBy(() -> createPaymentUseCase.pay(command))
// 				.isInstanceOf(RuntimeException.class)
