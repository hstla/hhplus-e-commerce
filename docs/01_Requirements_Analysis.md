...
2. [요구사항 분석 문서](#-요구사항-분석-문서)
    ...
    7. [예상 추가 기능](#-추가-기능-계획-혹은-선택-구현-항목)
3. [비즈니스 검증 사항](#-비즈니스-검증-사항)
4. [시퀀스 다이어그램](#-핵심-시퀀스-다이어그램)
5. [ERD](#-erd)
...
- 쿠폰 발급 조건:
    ...
    - 재고가 1 이상이어야 함
    - 만료일이 오늘 이후여야 함
    - 중복 발급 불가(한 유저가 한 쿠폰을 여러번 발급받는 것은 불가능합니다.)
...
1. 관리자가 쿠폰을 생성한다.(재고 50개)
2. 사용자 아이디와 쿠폰 아이디를 입력받아 쿠폰을 발급받을 수 있다. 이때 발급 쿠폰의 재고는 한 개이다.
3. 선착순 쿠폰이 유효한지와 쿠폰 재고가 있는지 확인한다.(실시간)
4. UserCoupon생성(id, couponId, userId, 사용/미사용여부)
5. 쿠폰 정보를 반환한다.
---
...
- **user**
  - name: 2 ~ 20글자
  - password: 5 ~ 30글자
  - point: 충전 시 1000원 이상 1000000원 이하

- **product**
  - name: 1 ~ 30글자
  - description: 10 ~ 200글자
