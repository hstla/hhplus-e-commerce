...
- 비효율적인 결과 확인: 클라이언트는 자신의 발급 성공 여부를 알기 위해 주기적으로 서버에 요청을 보내야 합니다(Polling). 이는 서버에 불필요한 부하를 유발합니다.

<br/>

### 개선: Redis & Kafka & SSE 기반 비동기 아키텍처
기존 아키텍처의 단점을 보안하고 시스템의 안정성과 확장성을 높이기 위해, 메시지 큐로 Kafka를, 실시간 알림 방식으로 SSE를 도입했습니다.
...
```mermaid
sequenceDiagram
participant Client as Client (SSE 연결)
participant APIServer as API 서버
participant Redis as Redis (재고 관리)
participant Kafka as Kafka (이벤트 브로커)
participant StockConsumer as 재고 차감 Consumer (1차 비동기 처리)
participant DBConsumer as DB 생성 Consumer (2차 영속화 처리)
...
Client-->>APIServer: taskId 기반 SSE 연결

Kafka-->>StockConsumer: 3. 이벤트 수신 (재고 감소)
...
DBConsumer-->>Client: SSE를 통해 발급 성공 알림 전송
```
### 장점
...
- 시스템 안정성(Reliability): Kafka의 로그 기반 저장 구조를 활용해 메시지 유실 위험을 낮추었습니다. 향후 재처리(리트라이) 및 데드레터 큐(DLQ) 도입을 통해 안정성을 추가 강화할 계획입니다.
- 선착순 정합성(Consistency): 대규모 동시 요청 속에서도 선착순이라는 비즈니스 로직이 깨지지 않도록 해야 했습니다. 이를 위해 Kafka Producer에서 쿠폰 ID를 파티션 키로 지정하여, 동일한 쿠폰 내 순서 보장을 확보했슴니다.
- 실시간 피드백(Real-time Feedback): 기존 Polling 방식은 서버 부하가 크고 지연이 심했기 때문에, 서버 푸시 기반 SSE로 전환하여 실시간 피드백을 제공했습니다.

## 4. 향후 과제 및 대응 방안
현재 아키텍처는 Redis 기반의 선착순 처리와 Kafka 비동기 이벤트 흐름을 통해 안정적으로 동작하고 있습니다.  
다만, 트래픽이 폭발적으로 증가할 경우 발생할 수 있는 병목 지점에 대비하기 위한 개선 방안을 고민했습니다.


### DB 쓰기 병목 문제
Redis 단계에서는 in-memory 연산으로 빠르게 처리되지만, DB는 I/O 바운드이기 때문에 트래픽 급증 시 컨슈머의 처리 속도가 밀릴 수 있습니다.
...
3. Kafka lag 모니터링을 통해 메시지 적체를 실시간으로 감지하고, 필요 시 컨슈머 인스턴스를 자동 확장(Scale-out)하도록 대응합니다.
